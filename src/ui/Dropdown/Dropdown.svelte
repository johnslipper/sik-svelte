...
<script lang="ts">
  ...
  function focusMenuItem(index: number) {
    ...
    const destinationItem = menuItems[index] as HTMLElement;
    if (destinationItem) {
  ...
  function menuTriggerKeyupListener(event: KeyboardEvent) {
  ...
  function menuItemKeydownListener(event: KeyboardEvent) {
    const keyboardKey = event.key;
    switch (keyboardKey) {
  ...
  onMount(() => {
    const handleOutsideClick = (event: MouseEvent) => {
      ...
      if ($menuIsVisible && !menuElement.contains(element)) {
        hideMenu();
      }
    };
